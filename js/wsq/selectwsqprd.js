/***
 * 商品选择插件
 * 调用1：在元素添加 class='j_select_wsqprd' 
 * 调用2：调用方法 selectWsqPrd.open(options) 变量可选，有默认
 	options = {
		ajaxUrl: 请求路径
		searchUrl: 搜索路径
		defaultId: 默认id
...
var selectWsqPrd = {};
(function(sobj) {
	var $modal = $('#selectWsqPrdModal'),
		$info = $modal.find('.j_info_bar'),
		$modalList = $modal.find('.j_list'), 
		$input = $modal.find('.j_searchinput'),
		$openbtn = $('.j_select_wsqprd'),
		pageBar = null, inited = false;
		nowpage = 1, lock = false;

	var options = { //默认参数
			ajaxUrl:  API.HOST + '/index.php/shangjia/wsqproduct/prdlist?community_id=' + API.community_id,
			searchUrl: API.HOST + '/index.php/shangjia/wsqproduct/search?community_id=' + API.community_id,
			defaultId: '',
	...
	sobj.getList = function(page, word){ //获取列表数据
		...
		if(word) url = options.searchUrl + '&title=' + word;
		sobj.word = word;
		$info.infobar('正在加载...');
		$.ajax({
			type: 'POST',
	...
	sobj.setList = function(data){ //列表dom操作
		...
		var tablehead = '<table class="table table-bordered table-striped"><tr><th width="200">商品名称</th><th width="100">参考价格</th><th width="100">操作</th></tr>',
			tablefoot = '</table>', tr = '';
		if(data && data.count){
			var list = data.prd_list;
			sobj.tempList = list;
			$.each(list, function(index, x){ //遍历json数据列
				var tclass = '';
				if(options.defaultId && x.prd_id == options.defaultId){
					x.selected = true;
				...
				tr += '<tr><td><span class="img"><img src="'+ x.prd_img +'"/></span><span class="name">'+ x.prd_name +'</span></td>';
				tr += '<td>'+ x.prd_price +'</td>';
				tr += '<td><span class="'+ tclass +'" onclick="selectWsqPrd.select(\'selectWsqPrd.tempList['+ index +']\')" data-id="'+ x.prd_id +'">选择</span></td>';
				tr += '</tr>';
			});
		}else{
			tablehead = '', tablefoot = '',
	...
	sobj.select = function(data){ //选中
		...
		//多个默认的时候在这扩展
		if(!options.defaultId || (options.defaultId && item.prd_id != options.defaultId)){
	...
	sobj.selectList = function(item){ //列表dom修改
		...
		$modal.find(".j_list .btn[data-id='" + item.prd_id + "']").addClass("btn-success");
	};
